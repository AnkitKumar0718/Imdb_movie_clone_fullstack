    ...
    <nav className='flex fixed-navbar items-center w-full bg-[#121212] h-16 px-4 md:gap-4'>
      ...
      <div className='relative'>
        ...
        {menuOpen && (
          <div className='absolute top-12 left-0 bg-[#242424] rounded shadow-lg z-10 py-2'>
            ...
            <RouterLink to='/wishlist' className='block px-4 py-2 text-white cursor-pointer hover:bg-gray-600' onClick={toggleMenu}>
              Wishlist
            </RouterLink>
            <div className='block px-4 py-2 text-white cursor-pointer hover:bg-gray-600' onClick={handleLogout}>
              Logout
            </div>
          </div>
        )}
      ...
      <form className='flex ml-4 relative flex-grow' onSubmit={search}>
        <input
          ...
          className='h-9 w-full md:w-[770px] pl-4 pr-10 rounded bg-white border border-gray-300 focus:ring-2 focus:ring-yellow-400 focus:border-transparent focus:outline-none'
      ...
      {/* Log Out button (Visible on medium screens and above) */}
      <p onClick={handleLogout} className='hidden md:block text-white font-bold hover:bg-[#242424] cursor-pointer py-2 px-2'>Log Out</p>
    </nav>
  );
};
